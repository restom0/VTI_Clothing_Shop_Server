...
public interface OnSaleProductRepository extends JpaRepository<OnSaleProduct, Long> {
	...
	@Query("SELECT onSaleProduct " +
			...
			/** Finds all by product id. */
			"AND (is.endDate IS NULL OR is.endDate >= CURRENT_TIMESTAMP)")
	List<OnSaleProduct> findAllByProductId(Long id);

	/** Finds by input sale id and input sale start date less than equal and deleted at is null and input sale deleted at is null order by id desc. */
	List<OnSaleProduct> findByInputSale_IdAndInputSale_StartDateLessThanEqualAndDeletedAtIsNullAndInputSale_DeletedAtIsNullOrderByIdDesc(
			Long inputSaleId, LocalDate startDate);
	...
	@Query("SELECT onSaleProduct " +
			...
			"AND onSaleProduct.deletedAt IS NULL " +
			"AND inputSale.deletedAt IS NULL " +
			/** Finds by product id and available date and null end date. */
			"AND inputSale.endDate IS NULL ")
	Optional<OnSaleProduct> findByProductIdAndAvailableDateAndNullEndDate(Long id,
	                                                                      @Future(message = "{messages.validation.future}")
	                                                                      @NotNull(message = "{messages.validation.required}")
	                                                                      LocalDate availableDate);

	/** Finds distinct by deleted at is null. */
