...
public interface OrderRepository extends JpaRepository<Order,Long> {
    ...
    @Override
    @Query("SELECT o FROM Order o WHERE o.deletedAt IS NULL ORDER BY o.id DESC")
    @NotNull
    List<Order> findAll();

    @Query("SELECT o FROM Order o WHERE o.deletedAt IS NULL AND o.user_id.id = ?1")
...
//    Order findOrderById(Long id);
    ...
    @Query("SELECT o FROM Order o WHERE o.deletedAt IS NULL AND o.id = ?1 AND o.user_id.id = ?2")
    Optional<Order> findOrderByIdAndUserId(Long id,Long userId);

    @Query("SELECT o FROM Order o WHERE o.deletedAt IS NULL AND o.user_id.id = ?1 AND o.payment_status = NOT_CONFIRMED")
