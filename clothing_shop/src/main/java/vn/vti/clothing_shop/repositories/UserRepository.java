...
public interface UserRepository extends JpaRepository<User,Long> {
    @Query("SELECT u FROM User u WHERE u.deletedAt IS NULL AND u.role = 'USER'")
    List<User> findAllUser();
    @Override
    @Query("SELECT u FROM User u WHERE u.id = ?1 AND u.deletedAt IS NULL ORDER BY u.id ASC")
    @NotNull
    ...
    @Query("SELECT u FROM User u WHERE u.phone_number = ?1 AND u.deletedAt IS NULL")
