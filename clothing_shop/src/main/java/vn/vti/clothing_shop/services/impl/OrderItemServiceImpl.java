...
public class OrderItemServiceImpl implements OrderItemService {
	...
	public void addOrderItem(OrderItemCreateRequest orderItemCreateRequest) throws WrapperException {
		try {
			Order order = orderRepository.findById(orderItemCreateRequest.orderId()).orElseThrow(
					() -> new NotFoundException("messages.orders.notfound"));
			OnSaleProduct onSaleProduct = onSaleProductRepository.findById(orderItemCreateRequest.productId()).orElseThrow(
					() -> new NotFoundException("messages.onSaleProducts.notfound"));
	...
	@Transactional
	@Override
	public void updateOrderItem(OrderItemUpdateRequest orderItemUpdateRequest, Long userId, Long orderId, Long orderItemId)
	...
	public void deleteOrderItem(Long id, Long orderId) throws WrapperException {
		try {
			...
			order.setTotalPrice(Math.max(NumberUtils.LONG_ZERO,
			                             order.getTotalPrice() - calculateLineTotal(orderItem.getQuantity(),
