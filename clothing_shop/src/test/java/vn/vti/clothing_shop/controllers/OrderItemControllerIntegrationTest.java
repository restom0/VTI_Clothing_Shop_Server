...
class OrderItemControllerIntegrationTest extends ControllerIntegrationTestSupport {
	...
	static Stream<ApiEndpoint> endpoints() {
		return Stream.of(
				...
				endpoint("GET /order-items/{orderId}/{id}", HttpMethod.GET, "/order-items/1/1", null, 200, Auth.USER, true,
				         false),
				endpoint("POST /order-items/", HttpMethod.POST, "/order-items/", orderItemCreateJson(), 201, Auth.USER, true,
				         false),
				endpoint("PUT /order-items/{orderId}/{id}", HttpMethod.PUT, "/order-items/1/1", orderItemUpdateJson(), 201,
				         Auth.USER, true, false),
				endpoint("DELETE /order-items/{orderId}/{id}", HttpMethod.DELETE, "/order-items/1/1", null, 200, Auth.USER, true,
				         false)
		);
	}
