...
CREATE TABLE IF NOT EXISTS imported_product (
    ...
    slider_url1 VARCHAR(255),
    slider_url2 VARCHAR(255),
    slider_url3 VARCHAR(255),
    slider_url4 VARCHAR(255),
    public_id_url VARCHAR(255),
    public_id_slider_url1 VARCHAR(255),
    public_id_slider_url2 VARCHAR(255),
    public_id_slider_url3 VARCHAR(255),
    public_id_slider_url4 VARCHAR(255),
    sku VARCHAR(255),
    gender VARCHAR(255),
...
CREATE TABLE IF NOT EXISTS input_sale (
    ...
    filter VARCHAR(255),
    filter_id BIGINT,
    sale_percentage REAL NOT NULL CHECK (sale_percentage >= 0),
...
CREATE TABLE IF NOT EXISTS on_sale_product (
    ...
    deleted_at BIGINT,
    version BIGINT,
    product_id BIGINT REFERENCES imported_product(id),
    input_sale_id BIGINT REFERENCES input_sale(id)
);
...
CREATE TABLE IF NOT EXISTS users (
    ...
    public_id_avatar_url VARCHAR(255),
...
INSERT INTO category (id, created_at, updated_at, version, name, description) VALUES
    (1, NOW(), NOW(), 0, 'T-Shirt', 'Everyday tops'),
    (2, NOW(), NOW(), 0, 'Jacket', 'Outerwear'),
    (3, NOW(), NOW(), 0, 'Pants', 'Bottoms')
ON CONFLICT DO NOTHING;

INSERT INTO brand (id, created_at, updated_at, version, name, description) VALUES
    (1, NOW(), NOW(), 0, 'VTI Basics', 'In-house essentials'),
    (2, NOW(), NOW(), 0, 'Urban Stitch', 'Casual streetwear')
ON CONFLICT DO NOTHING;

INSERT INTO product (id, created_at, updated_at, version, name, short_description, category_id, brand_id) VALUES
    (1, NOW(), NOW(), 0, 'Basic Cotton T-Shirt', 'Soft cotton crew neck', 1, 1),
    (2, NOW(), NOW(), 0, 'Denim Jacket', 'Classic denim outerwear', 2, 2)
ON CONFLICT DO NOTHING;

INSERT INTO color (id, created_at, updated_at, version, code, name, category_id) VALUES
    (1, NOW(), NOW(), 0, '#FFFFFF', 'White', 1),
    (2, NOW(), NOW(), 0, '#111111', 'Black', 1),
    (3, NOW(), NOW(), 0, '#2F5DA8', 'Denim Blue', 2)
ON CONFLICT DO NOTHING;

INSERT INTO size (id, created_at, updated_at, version, name, height, weight, category_id) VALUES
    (1, NOW(), NOW(), 0, 'M', '165-175cm', '55-70kg', 1),
    (2, NOW(), NOW(), 0, 'L', '175-185cm', '70-85kg', 1),
    (3, NOW(), NOW(), 0, 'M', '165-175cm', '55-75kg', 2)
ON CONFLICT DO NOTHING;

INSERT INTO material (id, created_at, updated_at, version, name, category_id) VALUES
    (1, NOW(), NOW(), 0, 'Cotton', 1),
    (2, NOW(), NOW(), 0, 'Denim', 2)
ON CONFLICT DO NOTHING;

INSERT INTO imported_product (
    id, created_at, updated_at, version, product_id, size_id, color_id, material_id,
    image_url, slider_url1, slider_url2, slider_url3, slider_url4,
    public_id_url, public_id_slider_url1, public_id_slider_url2, public_id_slider_url3, public_id_slider_url4,
...
INSERT INTO input_sale (id, created_at, updated_at, version, filter, filter_id, sale_percentage, discount, start_date, end_date) VALUES
    (1, NOW(), NOW(), 0, 'ALL', 0, 120, 10, CURRENT_DATE - 1, CURRENT_DATE + 30)
ON CONFLICT DO NOTHING;

INSERT INTO on_sale_product (id, created_at, updated_at, version, product_id, input_sale_id) VALUES
    (1, NOW(), NOW(), 0, 1, 1),
    (2, NOW(), NOW(), 0, 2, 1)
ON CONFLICT DO NOTHING;

INSERT INTO voucher (id, created_at, updated_at, version, code, available_date, end_date, input_stock, stock, voucher_value) VALUES
    (
        1,
        NOW(),
        NOW(),
...
INSERT INTO users (
    id, created_at, updated_at, version, name, username, password, birthday,
...
SELECT setval(pg_get_serial_sequence('imported_product', 'id'), COALESCE((SELECT MAX(id) FROM imported_product), 1), true);
SELECT setval(pg_get_serial_sequence('input_sale', 'id'), COALESCE((SELECT MAX(id) FROM input_sale), 1), true);
SELECT setval(pg_get_serial_sequence('on_sale_product', 'id'), COALESCE((SELECT MAX(id) FROM on_sale_product), 1), true);
