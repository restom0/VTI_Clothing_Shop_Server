...
If the cluster cannot read local Docker images directly, push the image to a registry and update `k8s/package/server.yaml`.

## Apply modules

Apply monitoring first so the application can export telemetry as soon as it starts:
...
Grafana uses the credentials from `k8s/monitoring/grafana-secret.yaml`.

## Notes

- `package` writes to PostgreSQL and MongoDB through the application configuration already present in `application.properties`.
- Nginx API Gateway proxies HTTP `/api/*` and `/` to `clothing-shop-server:8080`, and proxies gRPC module paths to `clothing-shop-server:9091`.
...
- Replace the dummy payment secrets in `k8s/package/app-secret.yaml` before using a shared or production cluster.
